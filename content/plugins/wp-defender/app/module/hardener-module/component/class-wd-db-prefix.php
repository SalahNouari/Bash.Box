...
class WD_DB_Prefix extends WD_Hardener_Abstract {
	...
	public function on_creation() {
		$this->id         = 'db_prefix';
		$this->title      = __( 'Change default database prefix', wp_defender()->domain );
	...
	private function validate_environment() {
		...
		if ( ! is_writable( $wp_config_path ) ) {
			$this->output_error( 0, __( "Your wp-config.php is not writable", wp_defender()->domain ) );
		}

		if ( ! function_exists( 'file' ) || ! function_exists( 'file_get_contents' ) || ! function_exists( 'file_put_contents' ) ) {
			$this->output_error( 1, sprintf( __( "This plugin will require these functions % enabled on your server", wp_defender()->domain ), 'file, file_get_contents, file_put_contents' ) );
		}
	...
	public function process() {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) ) {
			wp_send_json( array(
				'status' => 0,
				'error'  => __( "Table prefix can only contain numbers, letters, and underscores.", wp_defender()->domain )
			) );
		}
	...
	public function update_table_prefix( $prefix, $old_prefix = null ) {
		$this->log( 'Started rename tables', self::ERROR_LEVEL_DEBUG );
		global $wpdb;
		...
		foreach ( $tables as $table ) {
			...
			$this->log( sprintf( 'RENAME table %s to %s', $table, $new_table_name ), self::ERROR_LEVEL_DEBUG );
			$sql = "RENAME TABLE `{$table}` TO `{$new_table_name}`";
			if ( $wpdb->query( $sql ) === false ) {
				$this->log( __( 'RENAME table %s to %s error, message :%s query:%s', $table, $new_table_name, $wpdb->last_error, $wpdb->last_query ), self::ERROR_LEVEL_DEBUG );
				//revert here
		...
		$this->log( 'End rename tables successfully', self::ERROR_LEVEL_DEBUG );
	...
	public function update_wpconfig( $prefix, $path = null ) {
		...
		//write back
		$this->log( 'start update wp-config.php', self::ERROR_LEVEL_DEBUG );
		if ( ! file_put_contents( $path, $config_content, LOCK_EX ) ) {
			if ( $this->is_ajax() ) {
				$this->output_error( 'cant_write', __( "It seems the wp-config.php file is currently using by another process or isn't writeable.", wp_defender()->domain ) );
			} else {
				return false;
			}
		}
		$this->log( 'end update wp-config.php', self::ERROR_LEVEL_DEBUG );
	...
	public function update_table_data( $prefix, $old_prefix = null ) {
		$this->log( 'Started update tables data', self::ERROR_LEVEL_DEBUG );
		global $wpdb;
		if ( is_null( $old_prefix ) ) {
			$old_prefix = $wpdb->prefix;
		}
		if ( is_multisite() ) {
			...
			if ( is_array( $blogs ) && count( $blogs ) ) {
				foreach ( $blogs as $blog_id ) {
					...
					$this->log( sprintf( 'Fixing option %s', $prefix . $blog_id . '_user_roles' ), self::ERROR_LEVEL_DEBUG );
					if ( $wpdb->query( $sql ) == false ) {
						$this->log( sprintf( 'Fixing option %s error, message %s sql', $prefix . $blog_id . '_user_roles error', $wpdb->last_error, $sql ), self::ERROR_LEVEL_DEBUG );
		...
		foreach ( $rows as $row ) {
			if ( strpos( $row['meta_key'], $old_prefix ) === 0 ) {
				...
				//run the updater
				$this->log( sprintf( 'Fixing usermeta %s to %s', $row['meta_key'], $new_name ), self::ERROR_LEVEL_DEBUG );
				if ( $wpdb->query( $sql ) === false ) {
					$this->log( sprintf( 'Fixing usermeta %s to %s error. Message: %s', $row['meta_key'], $new_name, $wpdb->last_error ), self::ERROR_LEVEL_DEBUG );

					return $this->output_error( 'sql_error', $wpdb->last_error );
				}
			}
		}
		$this->log( 'End database update', self::ERROR_LEVEL_DEBUG );

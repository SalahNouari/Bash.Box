...
class WpmuDev_HelpTooltips {
	...
	function initialize() {
		...
		if ( ! $this->_inline_tips && ! $this->_bound_tips ) {
			return false;
		}
	...
	private function _check_screen() {
		...
		if ( ! is_object( $screen ) ) {
			return false;
		} // Actually older then 3.3
		if ( $this->_screen_id != @$screen->id ) {
			return false;
		} // Not for this screen
	...
	private function _print_styles() {
		...
		if ( ! defined( 'WPMUDEV_TOOLTIPS_CSS_ADDED' ) ) {
	...
	private function _print_scripts() {
		// Have we already done this?
		if ( ! defined( 'WPMUDEV_TOOLTIPS_JS_ADDED' ) ) {

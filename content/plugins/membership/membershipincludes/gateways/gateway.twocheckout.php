<?php
/*
Addon Name: 2Checkout Payment Gateway
Author: Incsub
...
class twocheckout extends Membership_Gateway {
	...
	function remove_recurring_line_item($sub_id, $user_id) {
		...
		$db_result = $this->db->get_row( $this->db->prepare( "SELECT transaction_paypal_ID, transaction_status FROM " . MEMBERSHIP_TABLE_SUBSCRIPTION_TRANSACTION . " WHERE transaction_subscription_ID = %s AND transaction_user_ID = %s AND transaction_gateway = %s ORDER BY transaction_ID DESC LIMIT 1", $sub_id, $user_id, $this->gateway ) );
		$invoice_id = ! empty( $db_result->transaction_paypal_ID ) ? $db_result->transaction_paypal_ID : false;
		$transaction_status = ! empty( $db_result->transaction_status ) ? $db_result->transaction_status : false;
		...
		if(empty($invoice_id) || !$invoice_id) {
			...
			if ( 'Processed' == $transaction_status ) {
				$args['body'] = array('invoice_id' => $invoice_id);
			} else {
				$args['body'] = array('sale_id' => $invoice_id);
			}
				...
				if ($response_obj->response_code == "OK" && $product_id != false) {
					...
					foreach($response_obj->sale->invoices as $invoice) {
						foreach($invoice->lineitems as $lineitem) {
							if($lineitem->vendor_product_id == $product_id)
	...
	function mysettings() {
		...
		<tbody>
		  <tr valign="top">
		  	<th scope="row" colspan="2">
				<div class="updated below-h2">
					<p><?php _e('In order for recurring payments to log properly you must enable your global instant notifications url pointing to:<br /><br /><strong>'.home_url('index.php?paymentgateway=' . esc_attr($this->gateway)).'&</strong> ', 'membership') ?></p>
					<p style="font-weight:normal; color:#808080;"><?php _e('Please make sure you add this to your 2Checkout "Notifications" as well as "Approved URL". The domain must be the same as the one registered with your Live account for production sites.')?></p>
				</div>
			</th>
		  </tr>
		  <tr valign="top">
		  ...
		  <option value="test" <?php if (get_option( $this->gateway . "_twocheckout_status" ) == 'test') echo 'selected="selected"'; ?>><?php _e('Sandbox Account', 'membership') ?></option>
		  </select>
		  <br />
		  </td>
		  </tr>
	 ...
	 */
	function get_product_id($subscription, $pricing, $noerror = false ) {
		$product_id = 0;
			...
			if (!$found) {
				if (count($pricing) < 2) {
					$bargs = array(
						...
						'approved_url' => trailingslashit(get_option('home')) . '?paymentgateway=twocheckout&' . esc_attr($this->gateway),
						'tangible' => 0,
	...
	function single_sub_button($pricing, $subscription, $user_id, $repeat = false) {
		...
		// Fetch product_id
		$product_id = $this->get_product_id($subscription, $pricing, true);

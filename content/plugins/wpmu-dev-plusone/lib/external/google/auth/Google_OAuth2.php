...
class Google_OAuth2 extends Google_Auth {
  ...
  public function __construct() {
    ...
    if (! empty($Gplus_apiConfig['developer_key'])) {
      $this->developerKey = $Gplus_apiConfig['developer_key'];
    }

    if (! empty($Gplus_apiConfig['oauth2_client_id'])) {
      $this->clientId = $Gplus_apiConfig['oauth2_client_id'];
    }

    if (! empty($Gplus_apiConfig['oauth2_client_secret'])) {
      $this->clientSecret = $Gplus_apiConfig['oauth2_client_secret'];
    }

    if (! empty($Gplus_apiConfig['oauth2_redirect_uri'])) {
      $this->redirectUri = $Gplus_apiConfig['oauth2_redirect_uri'];
    }

    if (! empty($Gplus_apiConfig['oauth2_access_type'])) {
      $this->accessType = $Gplus_apiConfig['oauth2_access_type'];
    }

    if (! empty($Gplus_apiConfig['oauth2_approval_prompt'])) {
      $this->approvalPrompt = $Gplus_apiConfig['oauth2_approval_prompt'];
    }
  }
